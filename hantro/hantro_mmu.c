...
#ifdef __KERNEL__
#define MMUDEBUG(fmt, args...) pr_info("hantrommu: " fmt, ##args)
#else
...
static enum MMUStatus CreateNode(struct MMUDDRRegion *region)
{
	...
	for (i = 0; i < MMU_REGION_COUNT; i++) {
		...
		switch (i) {
		case MMU_REGION_IN:
			page_count = (REGION_IN_END - REGION_IN_START + 1) /
				     PAGE_SIZE;
			p->stlb_start = region_in_mmu_start >> 12 &
					0x3FF; //hold mmu addr: 0x0
			...
			break;
		case MMU_REGION_OUT:
			page_count = (REGION_OUT_END - REGION_OUT_START + 1) /
				     PAGE_SIZE;
...
int hantroMMUprobe(dtbnode *pnode, enum MMUProbeStage loop, struct mmu_t *p,
			...
			struct mmu_core_cfg *cfg)
{
		...
		for (i = 0; i < mmu_core_num; i++) {
			...
			pointer =
				kmalloc(sizeof(struct MMU),
					GFP_KERNEL | MMU_NOWARN | __GFP_DMA32);
			if (!pointer) {
	...
	if (!request_mem_region(pmmu->core_cfg.mmucorebase,
				pmmu->core_cfg.iosize, "hantrommu")) {
		kfree(pmmu->g_mmu);
		return -ENODEV;
	}
...
#else
	AcquireMutex(mmucore->hw_mutex, MMU_INFINITE);
	iowrite32((MTLB_PCIE_START_ADDRESS >> 8) & 0xfffffff0,
...
static enum MMUStatus MMUMemNodeUnmap(struct addr_desc *addr, struct MMUDDRRegion *region, struct mmu_tbl_info *pg_tbl)
{
	unsigned int i;
	unsigned long long address = 0x0;
	unsigned int *page_table_entry;
...
static long MMUCtlBufferMap(struct file *filp, struct addr_desc *addr,
			    struct mmu_t *mmucore)
{
	MMUMemNodeMap(addr, filp, mmucore, 0);
	while (mmucore) {
		MMUCtlFlush(mmucore);
		mmucore = mmucore->next;
	}
...
static long MMUCtlBufferUnmap(struct addr_desc *addr, struct MMUDDRRegion *region, struct mmu_tbl_info *pg_tbl)
{
	MMUMemNodeUnmap(addr, region, pg_tbl);
	return 0;
}

static struct mmu_t *getMMUcore(struct addr_desc *addr)
{
...
void hantro_mmu_map(struct addr_desc *addr, int use_phyaddr)
{
	struct mmu_t *mmu;
...
long hantroMMUIoctl(unsigned int cmd, void *filp, unsigned long arg)
{
	...
	case HANTRO_IOCS_MMU_MEM_UNMAP:
		...
		if (!mmu)
			return -ENODEV;
		ret = MMUCtlBufferUnmap(&addr, &mmu->g_mmu->region[0], &mmu->pg_tbl);
		break;
