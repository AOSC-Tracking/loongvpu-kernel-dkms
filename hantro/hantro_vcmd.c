...
static u32 __maybe_unused vcmd_isr_polling = 1;

static irqreturn_t hantrovcmd_isr(int irq, void *dev_id);

#endif

/* debug use, yzk add code */
//#define PDEBUG(fmt, args...) pr_info("PDEBUG: " fmt, ##args)
...
#ifdef VSI_FPGA_MEM
static int init_vcmd_pool_on_hantro_fpgamem(vcmd_dev_str *subsys_dev);
static void release_vcmd_pool_on_hantro_fpgamem(vcmd_dev_str *subsys_dev);
	...
	if (subsys_core->working_state == WORKING_STATE_IDLE) {
		if ((first_linked_cmdbuf_node) &&
			...
			vcmd_write_reg((const void *)subsys_core->hwregs, 0x40,
				       subsys_core->reg_mirror[0x40 / 4]);
#ifdef VCMD_DEBUG_INTERNAL
			printk_vcmd_register_debug(subsys_core->hwregs,
						   "vcmd start exits ");
#endif
		}
	}
	...
	vcmd_dev_str *subsys_dev)
{
	...
	while (1) {
		if (!process_manager_node) {
			pr_err("hantrovcmd: ERROR process_manager_node, current progress is %s!!\n", current->comm);
			spin_unlock_irqrestore(
			   ...
			   u16 cmdbuf_id)
{
	...
	if (cmdbuf_obj->filp != filp) {
		pr_info("hantrovcmd: WARN cmdbuf_id %d not belong to this progress!!\n", cmdbuf_id);
		return -1;
	}
			   ...
			   u16 cmdbuf_id)
{
	...
	cmdbuf_obj = (struct cmdbuf_obj *)new_cmdbuf_node->data;
	if (cmdbuf_obj->filp != filp) {
		pr_err("hantrovcmd: ERROR cmdbuf_id with mismatch progress filp !!\n");
		return -1;
	}
	module_type = cmdbuf_obj->module_type;
	pmo = cmdbuf_obj->process_manager_obj;
			       ...
			       u32 *irq_status_ret)
{
	...
	for (k = 0; k < TOTAL_DISCRETE_CMDBUF_NUM; k++) {
		...
		if (subsys_core->vcmd_core_cfg.vcmd_irq == -1) {
			if (!cmdbuf_obj->cmdbuf_run_done) {
				hantrovcmd_isr(subsys_core->core_id,
								subsys_core);
			}
		}
				      ...
				      u32 *irq_status_ret)
{
	...
	if (cmdbuf_id != ANY_CMDBUF_ID) {
		...
		if (cmdbuf_obj->filp != filp) {
			//should not happen
			pr_err("hantrovcmd: ERROR cmdbuf_id with mismatch progess filp!!\n");
			return -1;
		}
		subsys_core = get_dev_of_core(subsys_dev, cmdbuf_obj->core_id);
...
static irqreturn_t hantrovcmd_isr(int irq, void *dev_id)
{
	...
	if (subsys_core->vcmd_core_cfg.vcmd_irq == -1)
		PDEBUG("hantrovcmd_isr:received fake IRQ!\n");
	else
	...
	if (vcmd_get_register_mirror_value(subsys_core->reg_mirror,
		...
		new_cmdbuf_node = subsys_core->list_manager.head;
		subsys_core->working_state = WORKING_STATE_IDLE;
		subsys_core->aborted_cmdbuf_id = cmdbuf_id;
		if (subsys_core->hw_version_id > HW_ID_1_0_C) {
			new_cmdbuf_node =
...
static void read_main_module_all_registers(vcmd_core_str *subsys_core)
{
	...
	hantrovcmd_isr(input_para.core_id, subsys_core);
	...
	} else if (subsys_dev->type == HANTRO_DECODER) {
		pr_info("vcmd_driver: main module register 0:0x%x\n",
			*status_base_virt_addr);
...
static int hantro_vcmd_mem_pool_init(vcmd_dev_str *subsys_dev)
{
	int ret;
	...
	if (ret != 0)
		return -1;
#else
	ret = init_vcmd_pool_on_hantro_fpgamem(subsys_dev);
	if (ret != 0)
		return -1;
#endif
...
static int hantro_vcmd_subsys_probe(vcmd_dev_str *subsys_dev)
{
	...
	for (i = 0; i < subsys_dev->subsys_num; i++) {
		...
		if (subsys_core->useirq &&
			...
			/* decoder and encoder use different irq name, yzk add code. */
			if (subsys_core->type == 2) {
				result =
					request_irq(subsys_core->vcmd_core_cfg.vcmd_irq,
						    hantrovcmd_isr,
						    IRQF_SHARED,
					            "hantro_vcmd_dec", (void *)subsys_core);
			}

			if (subsys_core->type == 4) {
				result =
					request_irq(subsys_core->vcmd_core_cfg.vcmd_irq,
						    hantrovcmd_isr,
						    IRQF_SHARED,
						    "hantro_vcmd_enc", (void *)subsys_core);
			}
...
static void hantro_vcmd_mem_pool_release(vcmd_dev_str *subsys_dev)
{
#ifndef VSI_FPGA_MEM
	customized_release_vcmd_mem_pool(subsys_dev);
#else
	release_vcmd_pool_on_hantro_fpgamem(subsys_dev);
#endif
...
static int hantro_vcmd_subsys_cleanup(vcmd_dev_str *subsys_dev)
{
	u32 i;
	u32 result;
	vcmd_core_str *subsys_core;
...
int hantro_vcmd_init(u32 sliceidx)
{
	...
	if (cur_slice->dec_vcmd.subsys_num != 0)
		hantro_vcmd_subsys_probe(&cur_slice->dec_vcmd);
	if (cur_slice->enc_vcmd.subsys_num != 0)
		hantro_vcmd_subsys_probe(&cur_slice->enc_vcmd);
	return 0;
}

int hantro_vcmd_probe(struct pci_dev *pdev, int useirq, struct device_node *slice, unsigned long ddr_base, unsigned long reg_base)
{
	vcmd_slice_str *cur_slice;

	cur_slice = hantro_vcmd_analyze_subnode(pdev, useirq, slice);
...
int hantro_vcmd_init(void *p)
{
	...
	int slice_num = 0;
	vcmd_slice_str *cur_slice;
	unsigned long dec_base_reg_hw;
	unsigned long enc_base_reg_hw;
	unsigned long base_ddr_hw;
	struct hantro_base_addr *base_addr = p;
	...
	for (i = 0; i < slice_num; i++) {
		...
		if (cur_slice->dec_vcmd.subsys_num != 0) {
			...
			if (hantro_vcmd_mem_pool_init(&cur_slice->dec_vcmd) < 0) {
				pr_err("%s,%d hantro_vcmd_mem_pool_init failed\n", __func__, __LINE__);
				return -1;
			}
			if (hantro_vcmd_subsys_probe(&cur_slice->dec_vcmd) < 0) {
				pr_err("%s,%d hantro_vcmd_subsys_probe failed\n", __func__, __LINE__);
				return -1;
			}
		}
		...
		if (cur_slice->enc_vcmd.subsys_num != 0) {
			...
			if (hantro_vcmd_mem_pool_init(&cur_slice->enc_vcmd) < 0) {
				pr_err("%s,%d hantro_vcmd_mem_pool_init failed\n", __func__, __LINE__);
				return -1;
			}
			if (hantro_vcmd_subsys_probe(&cur_slice->enc_vcmd) < 0) {
				pr_err("%s,%d hantro_vcmd_subsys_probe failed\n", __func__, __LINE__);
				return -1;
			}
		}
...
static int hantro_get_core_type(u32 sub_module_type)
{
	...
	case VCMD_TYPE_CUTREE:
		core_type = HANTRO_CORE_ENC;
		break;
	case VCMD_TYPE_JPEG_ENCODER:
		core_type = HANTRO_CORE_ENCJPG;
		break;
#endif
#ifdef HAS_VCD
	case VCMD_TYPE_DECODER:
		core_type = HANTRO_CORE_DEC;
		break;
	case VCMD_TYPE_JPEG_DECODER:
		core_type = HANTRO_CORE_DECJPG;
		break;
#endif
	default:
		core_type = -1;
	...
	unsigned long ddr_base, unsigned long enc_reg_base, unsigned long dec_reg_base, unsigned int enc_irq, unsigned int dec_irq)
{
	...
	for (i = 0; i < slice_num; i++) {
		...
		for (j = 0; j < core_num; j++) {
			...
			tmp.mmucorebase = reg_base + vcmd_cfg_p->vcmd_base_addr + vcmd_cfg_p->submodule_MMU_addr;	//todo yzk now we don't have mmu.
			tmp.sliceidx = i;
			if (vcmd_core->type == HANTRO_CORE_DEC)
				tmp.parenttype = HANTRO_CORE_DEC;
			else if (vcmd_core->type == HANTRO_CORE_ENC)
				tmp.parenttype = HANTRO_CORE_ENC;

			if (vcmd_cfg_p->submodule_MMU_addr != 0xffff)
				hantroMMUprobe(NULL, ALL_RESOURCE, NULL, pdev, ddr_base, &tmp);
#endif
		}
...
int hantro_vcmd_cleanup(void)
{
	...
	while (cur_slice) {
		if (cur_slice->dec_vcmd.subsys_num != 0) {
			hantro_vcmd_subsys_cleanup(&cur_slice->dec_vcmd);
			hantro_vcmd_mem_pool_release(&cur_slice->dec_vcmd);
		}
		if (cur_slice->enc_vcmd.subsys_num != 0) {
			hantro_vcmd_subsys_cleanup(&cur_slice->enc_vcmd);
			hantro_vcmd_mem_pool_release(&cur_slice->enc_vcmd);
		}
...
//TODO: refine enc/dec in one platform
	...
	for (i = 0; i < SUBS_DEV_COUNT; i++) {
		...
		//filp->private_data = (void *) subsys_core;
		process_manager_node = create_process_manager_node();
		if (!process_manager_node)
			return -1;
	...
	for (i = 0; i < SUBS_DEV_COUNT; i++) {
		...
		for (core_id = 0; core_id < total_vcmd_core_num; core_id++) {
				...
				if (subsys_core->hwregs && (cmdbuf_obj_temp->filp == filp)) {
					if (cmdbuf_obj_temp->cmdbuf_run_done) {
						...
						if (restart_cmdbuf == cmdbuf_obj_temp)
							restart_cmdbuf = new_cmdbuf_node->next ?
						...
						subsys_dev->software_triger_abort = 1;
	#ifdef VCMD_DEBUG_INTERNAL
						//printk_vcmd_register_debug((const void *)subsys_core->hwregs,"After trigger to 0");
	#endif
						...
						if (vcmd_get_register_value((const void *)subsys_core->hwregs,
							subsys_core->reg_mirror, HWIF_VCMD_IRQ_ABORT)) {
			...
			while (1) {
				if (!new_cmdbuf_node)
					break;
				pr_err("After: new_cmdbuf_node->data->filp = %p with current filp: %p\n", ((struct cmdbuf_obj *)new_cmdbuf_node->data)->filp, filp);
				new_cmdbuf_node = new_cmdbuf_node->next;
		}
	#endif
...
static unsigned long get_vcmd_pool_size(void)
{
...
static void get_vcmd_pool_mmuaddr(vcmd_dev_str *subsys_dev)
{
	...
	if (subsys_dev->vcmd_core->type == HANTRO_CORE_DEC)
		id = NODE_TYPE_DEC;
	else if (subsys_dev->vcmd_core->type == HANTRO_CORE_ENC)
		id = NODE_TYPE_ENC;

	addr.id = (subsys_dev->sliceidx << 16) | (id << 8);
	pr_info("%s addr sliceidx %d, %d\n", __func__, subsys_dev->sliceidx, id);
...
int vcmd_pm_resume(void *_dev)
{
	...
	for (i = 0; i < subsys_dev->subsys_num; i++) {
		...
		if (subsys_core->working_state == WORKING_STATE_IDLE) {
			if (node)
				vcmd_link_cmdbuf(subsys_core, node->next);
