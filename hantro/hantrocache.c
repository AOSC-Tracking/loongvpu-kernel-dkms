...
{
	struct slice_info *parentslice = getparentslice(dev, HANTRO_CORE_CACHE);
	unsigned long flags;
	int rdy = 0;
...
static long ReserveCore(struct cache_dev_t *dev, struct file *filp)
{
	...
	parentslice = getparentslice(dev, HANTRO_CORE_CACHE);
...
long hantrocache_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case CACHE_IOCG_CORE_NUM:
		id = arg;
		id = get_slicecorenum(id, HANTRO_CORE_CACHE);
		return id;
	case CACHE_IOCH_HW_RESERVE: {
		...
		while (pccore) {
			...
			if (pccore->core_cfg.client == client &&
			    ...
			    ((type == NODE_TYPE_DEC &&
			      pccore->parenttype == HANTRO_CORE_DEC) ||
			     (type == NODE_TYPE_ENC &&
			      pccore->parenttype == HANTRO_CORE_ENC)))
				break;
			pccore = pccore->next;
		}
		if (!pccore)
...
#if 0
		if (useirq && cachecore->irqlist[0] > 0) {
			result = request_irq(cachecore->irqlist[0], cache_isr,
					     IRQF_SHARED, "hantro_cache",
					     (void *)cachecore);
			if (result < 0) {
				pr_info("cachecore: request IRQ fail\n");
				ReleaseIO(cachecore);
				return -ENODEV;
			}
		}
...
static int ReserveIO(struct cache_dev_t *pccore)
{
	...
	if (!request_mem_region(pccore->core_cfg.base_addr,
				pccore->core_cfg.iosize, "hantro_cache")) {
		PDEBUG(KERN_INFO
		       "hantr_cache: failed to reserve HW regs,base_addr:%p\n",
	...
	pr_info("hantro_cache: hwid = %x, base_addr= %p\n", hwid,
		(void *)pccore->core_cfg.base_addr);
