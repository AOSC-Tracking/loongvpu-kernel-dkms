...
#define HANTRO_VCD_LAST_REG (HANTRO_VC9000D_REGS - 1)
...
static int irq[HXDEC_MAX_CORES] = { DEC_IRQ_0, DEC_IRQ_1, DEC_IRQ_0, DEC_IRQ_1,
			    DEC_IRQ_0, DEC_IRQ_1, DEC_IRQ_0, DEC_IRQ_1 };

static unsigned int iosize[HXDEC_MAX_CORES] = { DEC_IO_SIZE_0, DEC_IO_SIZE_1,
 ...
 */
static int sliceidxtable[HXDEC_MAX_CORES] = { 0, 0, 1, 1, 2, 2, 3, 3 };
...
#ifdef VSI_CONFIG_PM
static long DecRestoreRegs(struct hantrodec_t *dev, u32 id)
{
	long i;
...
static long DecStoreRegs(struct hantrodec_t *dev, u32 id)
{
	long i;
...
int dec_pm_resume(void *_dev)
{
	...
	while (dev) {
		if (dev->dec_owner) {
			...
			DecRestoreRegs(dev, dev->core_id);
		}
		dev = dev->next;
	}
...
static void ReadCoreConfig(struct hantrodec_t *dev)
{
	...
	if (IS_G1(dev->hw_id)) {
		...
		if (tmp)
			pr_info("hantrodec: regs[%d] bit %d high, core[%d] has H264\n", HANTRODEC_SYNTH_CFG, DWL_H264_E, c);
		dev->cfg |= tmp ? 1 << DWL_CLIENT_TYPE_H264_DEC : 0;

		tmp = (reg >> DWL_JPEG_E) & 0x01U;
		if (tmp)
			pr_info("hantrodec: regs[%d] bit %d high, core[%d] has JPEG\n", HANTRODEC_SYNTH_CFG, DWL_JPEG_E, c);
		dev->cfg |= tmp ? 1 << DWL_CLIENT_TYPE_JPEG_DEC : 0;
		tmp = (reg >> DWL_HJPEG_E) & 0x01U;
		if (tmp)
			pr_info("hantrodec: regs[%d] bit %d high, core[%d] has HJPEG\n", HANTRODEC_SYNTH_CFG, DWL_HJPEG_E, c);
		dev->cfg |= tmp ? 1 << DWL_CLIENT_TYPE_JPEG_DEC : 0;
		tmp = (reg >> DWL_MPEG4_E) & 0x3U;
		if (tmp)
			pr_info("hantrodec: regs[%d] bit %d high, core[%d] has MPEG4\n", HANTRODEC_SYNTH_CFG, DWL_MPEG4_E, c);
		dev->cfg |= tmp ? 1 << DWL_CLIENT_TYPE_MPEG4_DEC : 0;

		tmp = (reg >> DWL_VC1_E) & 0x3U;
		if (tmp)
			pr_info("hantrodec: regs[%d] bit %d high, core[%d] has VC1\n", HANTRODEC_SYNTH_CFG, DWL_VC1_E, c);
		dev->cfg |= tmp ? 1 << DWL_CLIENT_TYPE_VC1_DEC : 0;

		tmp = (reg >> DWL_MPEG2_E) & 0x01U;
		if (tmp)
			pr_info("hantrodec: regs[%d] bit %d high, core[%d] has MPEG2\n", HANTRODEC_SYNTH_CFG, DWL_MPEG2_E, c);
		...
		tmp = (reg & mask);
		if (tmp & (1 << DWL_VP8_E))
			pr_info("hantrodec: regs[%d] bit %d high, core[%d] has VP8\n", HANTRODEC_SYNTH_CFG_2, DWL_VP8_E, c);
		if (tmp & (1 << DWL_VP7_E))
			pr_info("hantrodec: regs[%d] bit %d high, core[%d] has VP7\n", HANTRODEC_SYNTH_CFG_2, DWL_VP7_E, c);
		if (tmp & (1 << DWL_WEBP_E))
			pr_info("hantrodec: regs[%d] bit %d high, core[%d] has WebP\n", HANTRODEC_SYNTH_CFG_2, DWL_WEBP_E, c);
		dev->cfg |= tmp ? 1 << DWL_CLIENT_TYPE_VP8_DEC : 0;

		tmp = (reg >> DWL_AVS_E) & 0x01U;
		if (tmp)
			pr_info("hantrodec: regs[%d] bit %d high, core[%d] has AVS\n", HANTRODEC_SYNTH_CFG_2, DWL_AVS_E, c);
		...
		if (tmp)
			pr_info("hantrodec: regs[%d] bit %d high, core[%d] has HEVC\n", HANTRODEC_CFG_STAT, DWL_G2_HEVC_E, c);
		dev->cfg |= tmp ? 1 << DWL_CLIENT_TYPE_HEVC_DEC : 0;

		tmp = (reg >> DWL_G2_VP9_E) & 0x01U;
		if (tmp)
			pr_info("hantrodec: regs[%d] bit %d high, core[%d] has VP9\n", HANTRODEC_CFG_STAT, DWL_G2_VP9_E, c);
		...
		if (tmp)
			pr_info("hantrodec: regs[%d] bit %d high, core[%d] has H264HIGH10\n", HANTRODEC_SYNTH_CFG, DWL_H264HIGH10_E, c);
		dev->cfg |= tmp ? 1 << DWL_CLIENT_TYPE_H264_MAIN10 : 0;

		if ((IS_VCD_1_1(vcd_hw_id))) {
			tmp = (reg >> DWL_JPEG_E_1_1) & 0x01U;
			if (tmp)
				pr_info("hantrodec: regs[%d] bit %d high, core[%d] has JPEG\n", HANTRODEC_SYNTH_CFG, DWL_JPEG_E_1_1, c);
			tmp = (reg >> DWL_HJPEG_E_1_1) & 0x01U;
			if (tmp)
				pr_info("hantrodec: regs[%d] bit %d high, core[%d] has HJPEG\n", HANTRODEC_SYNTH_CFG, DWL_HJPEG_E_1_1, c);
		} else {
			tmp = (reg >> DWL_JPEG_E_1_3) & 0x01U;
			if (tmp)
				pr_info("hantrodec: regs[%d] bit %d high, core[%d] has JPEG\n", HANTRODEC_SYNTH_CFG, DWL_JPEG_E_1_3, c);
			tmp = (reg >> DWL_HJPEG_E_1_3) & 0x01U;
			if (tmp)
				pr_info("hantrodec: regs[%d] bit %d high, core[%d] has HJPEG\n", HANTRODEC_SYNTH_CFG, DWL_HJPEG_E_1_3, c);
		}
		dev->cfg |= tmp ? 1 << DWL_CLIENT_TYPE_JPEG_DEC : 0;
		tmp = (reg >> DWL_MPEG4_E) & 0x3U;
		if (tmp)
		...
		if (tmp)
			pr_info("hantrodec: regs[%d] bit %d high, core[%d] has HEVC\n", HANTRODEC_SYNTH_CFG_3, DWL_HEVC_E, c);
		dev->cfg |= tmp ? 1 << DWL_CLIENT_TYPE_HEVC_DEC : 0;

		tmp = (reg >> DWL_VP9_E) & 0x07U;
		if (tmp)
			pr_info("hantrodec: regs[%d] bit %d high, core[%d] has VP9\n", HANTRODEC_SYNTH_CFG_3, DWL_VP9_E, c);
		dev->cfg |= tmp ? 1 << DWL_CLIENT_TYPE_VP9_DEC : 0;
		...
		if (tmp)
			pr_info("hantrodec: regs[%d] bit %d high, core[%d] has PP\n", HANTRODECPP_CFG_STAT, DWL_PP_E, c);
		dev->cfg |= tmp ? 1 << DWL_CLIENT_TYPE_PP : 0;
		...
		if (dev->its_aux_core_id) {
			...
			if (tmp)
				pr_info("hantrodec: regs[%d] bit %d high, core[%d] has H264\n", HANTRODEC_SYNTH_CFG_2, DWL_H264_PIPELINE_E, c);
			next->cfg |= tmp ? 1 << DWL_CLIENT_TYPE_H264_DEC : 0;

			tmp = (reg >> DWL_JPEG_PIPELINE_E) & 0x01U;
			if (tmp)
				pr_info("hantrodec: regs[%d] bit %d high, core[%d] has JPEG\n", HANTRODEC_SYNTH_CFG_2, DWL_JPEG_PIPELINE_E, c);
...
static void ReleaseDecoder(struct hantrodec_t *dev, long core)
{
	...
	if (status & HANTRODEC_DEC_E) {
		pr_info("hantrodec: DEC[%li] still enabled -> reset %x\n", core,
			status);
...
long hantrodec_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case _IOC_NR(HANTRODEC_IOC_MC_CORES):
		id = (u32)arg;
		id = get_slicecorenum(id, HANTRO_CORE_DEC);
		PDEBUG("cores=%d\n", id);
		return id;
	...
	case _IOC_NR(HANTRODEC_DEBUG_STATUS): {
		...
		for (i = 0; i < (int)slice; i++) {
			...
			if (!pcore)
				continue;
			parentslice = getparentslice(pcore, HANTRO_CORE_DEC);
...
static int ReserveIO(struct hantrodec_t *core, struct hantrodec_t **auxcore)
{
	...
	pr_info("hantrodec: VCD has build id 0x%08x\n",
	       readl(core->hwregs + HANTRODEC_HWBUILD_ID_OFF));
